...
export const getServerSideProps: GetServerSideProps = async (context) => {
  ...
  const response = await fetchGetCampaignDetails(id, context);
  ...
  if (response == null || response.shop_id != shop_id) {
    ...
    // Do not return `context` directly, as it is not serializable
    return {
      props: {
        apiResponse: response,
        campaign_num: id,
        // Only return serializable data here
      },
    };
  }
};
...
const DetailsCampaign = (
  { apiResponse, campaign_num }: { apiResponse: CampaignArgs; campaign_num: string },
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    if (id === "modify_campaign") {
      if (infoCheck(campaignArgs)) {
        const result = await fetchModifyCampaign(campaign_num, campaignArgs, context);
        alert(result.message);
        if (result.success) {
          router.push("/campaign/details/" + campaign_num);
        } else {
  ...
  return (
    ...
    >
      ...
      <div className="button-container w-full text-right">
        <button className="border m-2 mt-4 p-2 bg-red-500 text-white rounded-lg">삭제하기</button>
      </div>
    </DashboardContainer>
  );
};
